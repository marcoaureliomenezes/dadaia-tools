...
def test_quando_cria_2_topicos_com_chaining_eles_sao_criados(get_kafka):
    ...
    get_kafka.create_idempotent_topic(
        topic_name=topic_1
    ).create_idempotent_topic(topic_name=topic_2)
    time.sleep(0.2)
...
def test_quando_cria_topico_que_existe_por_padrao_nao_sobrescreve(get_kafka):
    ...
    get_kafka.create_idempotent_topic(
        topic_name=topic, num_partitions=prev_num_partitions
    )
    get_kafka.create_idempotent_topic(
        topic_name=topic, num_partitions=pos_num_partitions
    )
    time.sleep(0.2)
...
@mark.kafka_admin
def test_quando_cria_topico_que_existe_e_overwrite_entao_sobrescreve(
    get_kafka,
):
    topic = 'test_topic'
    prev_num_partitions = 1
    ...
    time.sleep(0.2)
    get_kafka.create_idempotent_topic(
        topic_name=topic, num_partitions=pos_num_partitions, overwrite=True
    )
    partitions = get_kafka.describe_topic(topic)['partitions']
    result_partitions = len(partitions)
...
def test_quando_cria_topico_com_configuracoes_especiais_entao_cria_topico_com_configuracoes_especiais(
    ...
    get_kafka.create_idempotent_topic(
        topic_name=topic, topic_config=topic_config, overwrite=True
    )
    time.sleep(0.2)
